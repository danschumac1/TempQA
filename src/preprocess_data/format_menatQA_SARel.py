# to run
# python ./src/preprocess_data/format_menatQA_SARel.py
import pandas as pd
...
def main():
    ...
    col_list = ['relevant_context', 'wrong_date_context']
    train_mixed = reshape_context(train, col_list)
    dev_mixed = reshape_context(dev, col_list)

    # Save the new datasets
    train_mixed.to_json("./data/datasets/MenatQA/final/train_SARel.jsonl", lines=True, orient='records')
    dev_mixed.to_json("./data/datasets/MenatQA/final/dev__SARel.jsonl", lines=True, orient='records')
